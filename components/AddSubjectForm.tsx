    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md relative transition-all duration-300">
        ...
        <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">
            {isEditMode ? 'Modifier le sujet' : 'Créer un nouveau sujet'}
        </h2>
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label htmlFor="subject-title" className="block text-sm font-medium text-gray-700 mb-2">Titre du sujet</label>
            <input
              id="subject-title"
              type="text"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              placeholder="Ex: Ma pire expérience au lit"
